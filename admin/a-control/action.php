...
//kategori bentuk pelanggaran
  ...
  function inputpel($con,$c_pelanggaran,$c_siswa,$c_kelas,$c_benpel,$bobot,$c_guru,$at,$tp,$nama_file){
  	$akh=mysqli_query($con,"INSERT INTO pelanggaran set c_pelanggaran='$c_pelanggaran',c_siswa='$c_siswa',c_kelas='$c_kelas',c_benpel='$c_benpel',bobot='$bobot',c_guru='$c_guru',at='$at',tp='$tp',photo='$nama_file' ");
  }
...
//kelas
  function addkelas($con,$c_kelas,$kelas,$jurusan,$wali){
    $akh=mysqli_query($con,"INSERT INTO kelas set c_kelas='$c_kelas',kelas='$kelas',jurusan='$jurusan',wali='$wali' ");
    session_start();
    $_SESSION['pesan']='tambah';
    header('location:../../kelas');
  }
  function editkelas($con,$c_kelas,$kelas,$jurusan,$wali){
    $akh=mysqli_query($con,"UPDATE kelas set kelas='$kelas',jurusan='$jurusan',wali='$wali' where c_kelas='$c_kelas' ");
    session_start();
    $_SESSION['pesan']='edit';
    header('location:../../kelas');
  }
...
//siswa
  function addsiswa($con,$c_siswa,$c_kelas,$nisn,$nama,$jk,$alamat,$tl,$ortu,$hp,$tinggal){
  ...
  function editsiswa($con,$c_siswa,$nisn,$nama,$jk,$alamat,$tl,$ortu,$hp,$tinggal,$status){
    $akh=mysqli_query($con,"UPDATE siswa set nisn='$nisn',nama='$nama',jk='$jk',alamat='$alamat',tl='$tl',ortu='$ortu',hp='$hp',tinggal='$tinggal',status='$status' where c_siswa='$c_siswa' ");
    session_start();
    $_SESSION['pesan']='edit';
  ...
  function hapusmutasimasuk($con,$c_siswa,$c_kelas){
   $akh=mysqli_query($con,"DELETE from pelanggaran where c_siswa='$c_siswa' ");
    $akh2=mysqli_query($con,"DELETE from orangtua where c_siswa='$c_siswa' ");
    $akh3=mysqli_query($con,"DELETE from siswa where c_siswa='$c_siswa' ");
	 $akh4=mysqli_query($con,"DELETE from mutasi where c_siswa='$c_siswa' ");
    session_start();
    $_SESSION['pesan']='hapus';
    header('location:../../mmasuk');
  }
   function hapusmutasikeluar($con,$c_siswa,$c_kelas){
    $akh3=mysqli_query($con,"update siswa set status='Aktif' where c_siswa='$c_siswa' ");
	 $akh4=mysqli_query($con,"DELETE from mutasi where c_siswa='$c_siswa' ");
    session_start();
    $_SESSION['pesan']='hapus';
    header('location:../../mkeluar');
  }

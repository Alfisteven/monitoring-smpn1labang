        ...
        <?php $_SESSION['pesan'] = '';?>
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title"></span>&nbsp;Laporan Ketidakhadiran Siswa</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body table-responsive">
              <table id="example1" class="table table-bordered table-hover">
                ...
                <tbody>
<?php $smk=mysqli_query($con,"SELECT * FROM kelas order by kelas asc ");$vr=1;while($akh=mysqli_fetch_array($smk)){ $jsis=mysqli_num_rows(mysqli_query($con,"SELECT * from siswa where c_kelas='$akh[c_kelas]' ")); $jpe=mysqli_num_rows(mysqli_query($con,"SELECT * from pelanggaran where c_kelas='$akh[c_kelas]' and tp='$tp' and c_benpel='LBUjYfx7X'")); $ppe=mysqli_fetch_array(mysqli_query($con,"SELECT sum(bobot) as poin from pelanggaran where c_kelas='$akh[c_kelas]' and tp='$tp' and c_benpel='LBUjYfx7X' "));  ?>
                <tr>
                  <td><?php echo $vr; ?></td>
                  ...
                  <td align="center">
                  <a class="btn btn-success btn-sm" href="<?php echo $basead; ?>kehadiran_siswa/<?php echo $akh['c_kelas']; ?>"><i class="glyphicon glyphicon-education"></i> Lihat Per Siswa</a>
                  <a target="_blank" class="btn bg-blue btn-sm" href="<?php echo $basead.'print/printketidakhadiran?kelas='.$akh['c_kelas'].'&tp='; ?><?php echo "$tp";?>"><i class="glyphicon glyphicon-print"></i> Print</a></td>
                </tr>
      ...
      <div class="modal-header">
        ...
        <div class="modal-body">
          <div class="form-group">
            <label>KELAS</label>
            <input type="text" name="kelas" class="form-control" value="<?php echo $akh['kelas']; ?>"><br />

            <label>JURUSAN</label>
            <input type="text" name="jurusan" class="form-control" value="<?php echo $akh['jurusan']; ?>"><br />

            <label>WALI KELAS</label>
            <input type="text" name="wali" class="form-control" value="<?php echo $akh['wali']; ?>">
